...
<head>
    ...
    <title>StayFlow — Modifier Évaluation</title>
...
<div class="main">
    <div class="topbar">
        <h1><i class="fas fa-edit" style="color:#fb8c00"></i> Modifier Évaluation</h1>
    ...
    <div class="card">
        <form method="POST"
              action="{{ route('admin.evaluations.update', $evaluation->id_evaluation) }}">
            @csrf
            @method('PUT')
                ...
                <select name="id_client" required>
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id_client }}"
                            {{ $evaluation->id_client == $client->id_client ? 'selected' : '' }}>
                ...
                <select name="id_employe" required>
                    ...
                    @foreach($employes as $employe)
                        <option value="{{ $employe->id_employe }}"
                            {{ $evaluation->id_employe == $employe->id_employe ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="note" required>
                    ...
                    <option value="1" {{ $evaluation->note == 1 ? 'selected' : '' }}>★ — Très mauvais</option>
                    <option value="2" {{ $evaluation->note == 2 ? 'selected' : '' }}>★★ — Mauvais</option>
                    <option value="3" {{ $evaluation->note == 3 ? 'selected' : '' }}>★★★ — Moyen</option>
                    <option value="4" {{ $evaluation->note == 4 ? 'selected' : '' }}>★★★★ — Bien</option>
                    <option value="5" {{ $evaluation->note == 5 ? 'selected' : '' }}>★★★★★ — Excellent</option>
                </select>
            ...
            <div class="form-group">
                ...
                <textarea name="commentaire"
                          placeholder="Commentaire sur le service reçu...">{{ old('commentaire', $evaluation->commentaire) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input type="date" name="date"
                       value="{{ old('date', $evaluation->date->format('Y-m-d')) }}" required/>
