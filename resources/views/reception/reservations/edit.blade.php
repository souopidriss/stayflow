...
<head>
    ...
    <title>StayFlow — Modifier Réservation</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .info-box {
            ...
            font-size: 14px; border-left: 4px solid #1E88E5;
        }
        .info-box strong { color: #0F4C75; }
        .btn-logout {
...
<div class="main">
    <div class="topbar">
        <h1><i class="fas fa-edit" style="color:#1E88E5"></i>
            Modifier Réservation #{{ $reservation->id_reservation }}
        </h1>
    ...
    <div class="card">
        <div class="info-box">
            ...
            &nbsp;|&nbsp;
            <strong>Chambre :</strong> N° {{ $reservation->chambre->numero }}
            — {{ $reservation->chambre->typeChambre->libelle_type }}
        </div>

        <form method="POST"
              action="{{ route('reception.reservations.update', $reservation->id_reservation) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" name="date_arrivee"
                           value="{{ old('date_arrivee', $reservation->date_arrivee->format('Y-m-d')) }}"
                           required/>
                    @error('date_arrivee')
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="date_depart"
                           value="{{ old('date_depart', $reservation->date_depart->format('Y-m-d')) }}"
                           required/>
                    @error('date_depart')
                        <div class="error-msg">{{ $message }}</div>
                    @enderror
